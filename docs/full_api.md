...
The campaign_sequential_audiences are first party segment - sequential audiences that have been added to a campaign.

Refer to Sequential Audiences for details of how to find the available Sequential Audiences to add to a campaign as a retargeting segment.

NOTE: The segment_type_id must be set to 1

Listing campaign_sequential_audiences for a campaign:
Request

curl -s -X GET -H "X-App-Key: xxxx-xx-xx-xx-xxxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxxx" \
  -H "Content-Type: application/json" \
  "https://app.simpli.fi/api/campaigns/904/campaign_sequential_audiences"

Response

{
  "campaign_sequential_audiences": [
    {
      "resource": "https://app.simpli.fi/api/campaign_sequential_audiences/1",
      "id": 1,
      "campaign_id": 904,
      "segment_type_id": 1,
      "warning_message": null,
...
The warning_message field contains details of any issues with the sequential_audiences in the campaign. The field is null if there are no issues.
HTTP Methods Supported

    GET
    PUT

Changes to Campaign Sequential Audiences

The change endpoint is used for all modifications to the campaign_sequential_audiences. This includes adding, updating, and deleting the campaign_sequential_audiences on a campaign. It does not allow modification of the actual event that is used by the campaign. The change endpoint allows bulk actions to be performed. Multiple campaign_sequential_audiences can be added, updated, or removed in one action.

The example payload below does the following:

    Add two sequential_audiences to the campaign.
        sequential_id 123 is added. It is set with segment_target_type_id of 2 for 'Include' or 'Opt-in'.
        sequential_id 456 is added.
    Remove campaign_sequential_audiences with ids 789 and 987. These are the campaign_sequential_ids not the sequential_id.
    Update campaign_event with id 135. The segment_target_type_id is changed to 2 for 'Include'. Again, these are the campaign_sequential_ids not the sequential_id.
    Set the segment_match_type field to any which means that if any of the sequential_audiences match a user it is considered a match. The site_retargeting_segments_match_type field is also a valid parameter in this case but it has been deprecated in favor of segment_match_type.
...
curl -s -X PUT -H "X-App-Key: xxxx-xx-xx-xx-xxxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxxx" \
  ...
  "https://app.simpli.fi/api/campaigns/904/campaign_sequential_audiences/change"
...
Only Adding Sequential Audiences to the Campaign

This example adds three additional sequential_audiences.
Request
  ...
  -d '{
        "campaign_first_party_segments": {
          "add": [
            ...
            {
              "sequential_id": 1234567,
              "segment_target_type_id": 2,
              "segment_type_id": 1
            },
            {
              "sequential_id": 12345678,
              "segment_target_type_id": 2,
              "segment_type_id": 1
            }
          ]
...
Campaigns can be retrieved directly by ID without specifying the organization: GET /api/campaigns/{id}

The legacy format with organization context ( /api/organizations/{org_id}/campaigns/{id} ) is also supported for backwards compatibility.

Example requests:

# New format - direct campaign access
curl -i -H "X-User-Key: xxxx-xx-xx-xx-xxxxxx" \
...
Example for organization 379297, campaign 9:
Request

curl -i -X GET -H "X-App-Key: xxxx-xx-xx-xx-xxxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxxx" \
  -H "Content-Type: application/json" \
  "https://app.simpli.fi/api/organizations/379297/campaigns/9/custom_field_values"
...
Example for organization 379297, campaign 9, setting Impression Goal to 559. Note that a custom field with a name of "Impression Goal" must exist for your organization. See Custom Fields for more information.
Request
...
curl -i -X PUT -H "X-App-Key: xxxx-xx-xx-xx-xxxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxxx" \
  ...
  "https://app.simpli.fi/api/organizations/379297/campaigns/9/custom_field_values/replace"
...
Example for organization 379297:
Request

curl -i -X GET -H "X-App-Key: xxxx-xx-xx-xx-xxxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxxx" \
  -H "Content-Type: application/json" \
  "https://app.simpli.fi/api/organizations/379297/custom_fields"
...
Example for organization 379297, custom field 13:
Request

curl -i -X DELETE -H "X-App-Key: xxxx-xx-xx-xx-xxxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxxx" \
  -H "Content-Type: application/json" \
  "https://app.simpli.fi/api/organizations/379297/custom_fields/13"
  ...
  "demographics": [
    {
      ...
      "field": "gender_female",
      "group": "Gender",
      "description": "A description of the demographic...",
...
ZIP Adviser - Rank (Campaign-Specific)

Get the ZIP Adviser results for the provided filters for a specific campaign. In order to assign any of these zip codes to the desired campaign, call the API endpoint specified in the response as `update_url` (documentation)
Request

curl -i -X POST -H "X-App-Key: xxxx-xx-xx-xx-xxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxx" \
  -H "Content-Type: application/json" \
  -d '{
        "data": {
          "gender": "Male",
          "ages": ["pop_35_44", "pop_55_64"],
          "income": ["100_125", "125_150", "150_200"],
          "metros": ["Orlando-Daytona Beach-Melbourne, FL"],
          "counties": [],
          "cities": ["Jacksonville, FL"],
          "zip_codes": [],
          "days": ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"],
          "dayparts": ["Prime Time", "Early Fringe"],
          "home_values": ["home_value_100k_125k", "home_value_125k_150k"],
          "interests": ["Careers", "Medical Health", "TV And Movies", "Sports"]
        }
      }' \
  "https://app.simpli.fi/api/organizations/8/campaigns/644/postal_codes/rank"

Response

{
  "update_url": "https://app.simpli.fi/api/organizations/8/campaigns/644",
  "data": [
    {
      "postal_code": "34786",
      "city": "Windermere",
      ...
      "rank": 100.0,
      "total_population": 13257,
      "households": 14869,
      "median_income": 105774,
      "ctv_count": 322145,
      "total_count": 1295659,
      "pop_total": 46817,
      "ctv_percent": 59.758402,
      "target_percent": 28.316637118995235
    },
    {
      "postal_code":"34638",
      "city":"Land O Lakes",
      ...
      "rank":70.54053705254026,
      "total_population":8851,
      "households":10292,
      "median_income":104038,
      "ctv_count":434575,
      "total_count":1416235,
      "pop_total":32345,
      "ctv_percent":60.261705,
      "target_percent":27.364353068480447
    },
    ...
  ]
}

ZIP Adviser - Rank (Standalone)

Get the ZIP Adviser results for the provided filters. This is a standalone endpoint that provides ZIP code rankings based on demographic filters. Note: This endpoint is not campaign-specific and does not include an update_url in the response.
Request
...
curl -i -X POST -H "X-App-Key: xxxx-xx-xx-xx-xxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxx" \
  ...
  "https://app.simpli.fi/api/zip_adviser/rank"
...
ZIP Adviser - Estimate

Get campaign reach and impression estimates based on demographic filters and targeting criteria. This endpoint provides estimated performance metrics for campaign planning and optimization.
Request
...
curl -i -X POST -H "X-App-Key: xxxx-xx-xx-xx-xxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxx" \
  ...
  -d '{
        "data": {
          ...
          "gender": "Male",
          "income": ["85_100"],
          "metros": ["Dallas-Ft. Worth, TX"],
          "counties": [],
          "cities": [],
          "zip_codes": [],
          "days": ["Wednesday", "Thursday", "Tuesday"],
          "dayparts": ["Daytime", "Prime Access"],
          "home_values": ["home_value_20k_25k", "home_value_25k_30k"],
          "interests": ["Medical Health", "Music and Audio"],
          "daily_budget": 1000,
          "max_bid": 10
        }
      }' \
  "https://app.simpli.fi/api/zip_adviser/estimate"
...

{
  "data": {
    "general": [
      {"label": "Total Population", "value": 980468},
      {"label": "Target Population", "value": 69196},
      {"label": "Total Zips", "value": 52},
      {"label": "Total Households", "value": 365160},
      {"label": "Avg. Household Size", "value": 2.69},
      {"label": "Median Income", "value": 91407, "prefix": "$"}
    ],
    "reach": [
      {"label": "CTV Viewership %", "value": 74.3, "suffix": "%"},
      {"label": "Linear Viewership %", "value": 23.5, "suffix": "%"},
      {"label": "GRPs", "value": 821.6},
      {"label": "Reach", "value": 74.33, "suffix": "%"},
      {"label": "CPP", "value": 36.51, "prefix": "$"},
      {"label": "Impressions", "value": 3000000},
      {"label": "Spend", "value": 30000.0, "prefix": "$"},
      {"label": "Frequency", "value": 11.05}
    ]
  }
}

Parameters (All ZIP Adviser Endpoints)

ages
    Array of pre-defined age bands:
...
gender
    All, Female, or Male 
income
    Array of pre-defined income bands:

        00_35 (Less than $35,000)
        35_45 ($35,000 - $44,999)
        45_55 ($45,000 - $54,999)
        55_70 ($55,000 - $69,999)
        70_85 ($70,000 - $84,999)
        85_100 ($85,000 - 99,999$)
        100_125 ($100,000 - $124,999)
        125_150 ($125,000 - $149,999)
        150_200 ($150,000 - $199,999)
        200_250 ($200,000 - $249,999)
        250_500 ($250,000 - $499,999)
        500_plus ($500,000+)

zip_codes
    Array of ZIP codes to target
metros
    Array of Metro areas to target. See associated API-endpoint below.
counties
    Array of Counties to target. See associated API-endpoint below.
cities
    Array of Cities to target. See associated API-endpoint below.
days
    Array of day targeting options:
...
dayparts
    Array of daypart targeting options:

        Late Fringe (11:30 PM - 12:59 AM)
        Post Late Fringe (1:00 AM - 1:59 AM)
        Overnight (2:00 AM - 5:59 AM)
        Early Morning (6:00 AM - 9:59 AM)
        Daytime (10:00 AM - 4:29 PM)
        Early Fringe (4:30 PM - 7:29 PM)
        Prime Access (7:30 PM - 7:59 PM)
        Prime Time (8:00 PM - 10:59 PM)
        Late News (11:00 PM - 11:29 PM)

home_values
    Array of home value ranges (use the ID values):

        home_value_less_10k (Less Than $10,000)
        home_value_10k_15k ($10,000 - $14,999)
        home_value_15k_20k ($15,000 - $19,999)
        home_value_20k_25k ($20,000 - $24,999)
        home_value_25k_30k ($25,000 - $29,999)
        home_value_30k_35k ($30,000 - $34,999)
        home_value_35k_40k ($35,000 - $39,999)
        home_value_40k_50k ($40,000 - $49,999)
        home_value_50k_60k ($50,000 - $59,999)
        home_value_60k_70k ($60,000 - $69,999)
        home_value_70k_80k ($70,000 - $79,999)
        home_value_80k_90k ($80,000 - $89,999)
        home_value_90k_100k ($90,000 - $99,999)
        home_value_100k_125k ($100,000 - $124,999)
        home_value_125k_150k ($125,000 - $149,999)
        home_value_150k_175k ($150,000 - $174,999)
        home_value_175k_200k ($175,000 - $199,999)
        home_value_200k_250k ($200,000 - $249,999)
        home_value_250k_300k ($250,000 - $299,999)
        home_value_300k_400k ($300,000 - $399,999)
        home_value_400k_500k ($400,000 - $499,999)
        home_value_500k_750k ($500,000 - $749,999)
        home_value_750k_1000k ($750,000 - $999,999)
        home_value_1000k_1500k ($1,000,000 - $1,499,999)
        home_value_1500k_2000k ($1,500,000 - $1,999,999)
        home_value_more_2000k ($2,000,000+)
...
daily_budget
    Daily budget amount in dollars (estimate endpoint only)
max_bid
    Maximum bid amount in dollars (estimate endpoint only)

ZIP Adviser - Metros

Get list of available Metros, grouped by State. This is a standalone endpoint for retrieving metro area information.
Request

curl -i -X GET -H "X-App-Key: xxxx-xx-xx-xx-xxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxx" \
  -H "Content-Type: application/json" \
  "https://app.simpli.fi/api/zip_adviser/metros?state=Florida"
...
state
    If a state is provided, the response will return only metros in that state

ZIP Adviser - Counties

Get list of available Counties, grouped by State. This is a standalone endpoint for retrieving county information.
Request

curl -i -X GET -H "X-App-Key: xxxx-xx-xx-xx-xxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxx" \
  -H "Content-Type: application/json" \
  "https://app.simpli.fi/api/zip_adviser/counties?state=Florida"
...
state
    If a state is provided, the response will return only counties in that state

ZIP Adviser - Cities

Get list of available Cities, grouped by State. This is a standalone endpoint for retrieving city information.
Request

curl -i -X GET -H "X-App-Key: xxxx-xx-xx-xx-xxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxx" \
  -H "Content-Type: application/json" \
  "https://app.simpli.fi/api/zip_adviser/cities?state=Florida"
...
state
    If a state is provided, the response will return only cities in that state
...
First Party Segment - Sequential Audiences

Listing sequential_audiences that belong to an organization.
Request

curl -i -X GET -H "X-App-Key: xxxx-xx-xx-xx-xxxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxxx" \
  -H "Content-Type: application/json" \
  "https://app.simpli.fi/api/organizations/5787/sequential_audiences"
...

{
  "first_party_segments": [
    {
      "resource": "http://localhost:3000/api/organizations/0/sequential_audiences/18",
      "id": 18,
      "name": "First Audience",
      "user_count": 0,
      "days_inactive": 0,
      ...
      "sequential_audience": {
        "id": 6,
        "rule_type": "impressions",
        "rule_threshold": 50,
        "rule_recency": 60,
        "created_at": "2024-07-30T18:07:13.729-05:00",
        "campaigns": [
          {
            "id": 2,
            "name": "Another camp",
            ...
            "type": null,
            "company_id": 0,
            "sequential_audience_campaign_id": 8
          }
        ],
        "trend": [
          0,
      ...
      "resources": [
        {
          "changes": "http://localhost:3000/api/organizations/0/sequential_audiences/18/changes"
        }
      ]
    },
    ... Not showing all sequential audiences in the output ...
  ],
  "paging": {
    "page": 1,
    "size": 10,
    "total": 2908,
    "next": "https://app.simpli.fi/api/organizations/5787/sequential_audiences?page=2"
  }
}
...
The search_term query string param looks for matches against the name of the sequential_audience.

Example:

curl -i -X GET -H "X-App-Key: xxxx-xx-xx-xx-xxxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxxx" \
  -H "Content-Type: application/json" \
  "https://app.simpli.fi/api/organizations/5787/sequential_audiences?search_term=hat"

Available First Party Segmnet - Sequential Audiences

The available endpoint will return all sequential_audiences that an organization may use in campaigns. These include the sequential_audiences owned by the organization and sequential_audiences owned by parent organizations.

The query string parameters defined above may be used with the available endpoint.

curl -i -X GET -H "X-App-Key: xxxx-xx-xx-xx-xxxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxxx" \
  -H "Content-Type: application/json" \
  "https://app.simpli.fi/api/organizations/5787/sequential_audiences/available"

Creating a First Party Segment - Sequential Audience

The POST body should be formatted as JSON and use the following format. The POST is only supported when nested within an organization context.

Valid values for rule_type:
...
Valid values for rule_threshold must be greater than 0.
When rule_type is audio_completions or video_completions:
...
Create a First Party Segment - Sequential Audience Example
...
curl -i -X POST -H "X-App-Key: xxxx-xx-xx-xx-xxxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxxx" \
  ...
  -d '{
        "first_party_segment": {
          "name": "API Audience",
          "sequential_audience_attributes": {
            "rule_type": "impressions",
            "rule_threshold": "1",
            "rule_recency": "30",
            "sequential_audience_campaigns_attributes": [
              { "campaign_id": 87 }
            ]
          }
        }
...
Getting a Specific First Party Segment - Sequential Audience

Example - get sequential_audience 18:

curl -i -X GET -H "X-App-Key: xxxx-xx-xx-xx-xxxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxxx" \
  -H "Content-Type: application/json" \
  "https://app.simpli.fi/api/sequential_audiences/25"
...

{
  "first_party_segments": [
    {
      "resource": "http://localhost:3000/api/organizations/0/sequential_audiences/25",
      "id": 25,
      "name": "API Audience",
      "user_count": 0,
      "days_inactive": 0,
        ...
        "campaigns": [
          {
            ...
            "type": null,
            "company_id": 0,
            "sequential_audience_campaign_id": 61
          }
        ],
        "trend": [
          0,
...
Updating a First Party Segment - Sequential Audience

The name field of the first_party_segment may be directly updated.

Update the sequential_audience_attributes by passing in the id of the associated sequential_audience.

Add source campaigns to the sequential_audience_campaigns_attributes using the campaign_id.

Remove source campaigns from the sequential_audience_campaigns_attributes using the sequential_audience_campaign_id of the associated campaign and setting _destroy: true.

Example - Updating sequential_audience with id 25:

curl -i -X PUT -H "X-App-Key: xxxx-xx-xx-xx-xxxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
        "first_party_segment": {
          "name": "Updated API Audience",
          "sequential_audience_attributes": {
            "id": 6,
            "rule_type": "clicks",
            "rule_threshold": "10",
  ...
  "https://app.simpli.fi/api/sequential_audiences/105235"

Deleting a First Party Segment - Sequential Audience

Deleting a sequential_audience is permanent. Deleting a sequential_audience can adversely affect the performance of any campaign using that sequential_audience. You will not be able to delete a sequential_audience currently in use by an active campaign. It must be removed from the campaign first.

Example - delete the sequential_audience with id 25:

curl -i -X DELETE -H "X-App-Key: xxxx-xx-xx-xx-xxxxxx" -H "X-User-Key: xxxx-xx-xx-xx-xxxxxx" \
  -H "Content-Type: application/json" \
  "https://app.simpli.fi/api/sequential_audiences/25"

Example - attempt to delete the sequential_audience with id 25 and the name "Updated API Audience" which is in use by an active campaign with a company_id of 92356 and id of 316772:
...

{
  "errors": [
    {
      "message": "The Updated API Audience (id: 25) audience is currently being used by campaigns, remove the audience from:"
      "campaign_error_urls": [
        "https://app.simpli.fi/organizations/92356/campaigns/316772/edit"
      ]
    }
